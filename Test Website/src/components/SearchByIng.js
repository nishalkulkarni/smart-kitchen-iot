...
export default function SearchByIng(props) {
    ...
    return (
       <Grid item xs={12}>
       <div>
           <h1 style={{fontFamily : "Verdana, Geneva, Tahoma, sans-serif"}}>Search For Recipe using Ingredients!</h1>
            <form >
                <label style={{fontFamily : "Verdana, Geneva, Tahoma, sans-serif"}}>Enter ingredient to add : </label>
                <input id="inputIngredient" type="text" value={searchTerm} onChange={handleChange}></input>
            ...
            <div className="ingredients-div">
                    <h2>Ingredients:</h2>
                    {ingred && ingred.map(ingredient => {return <div style = {ingredientCard}>{ingredient}<button onClick={(e)=>removeIngred(e,ingredient)} style={removeButton}> - </button></div>})}
                </div>
