...
export default function HeatIndexGauge({heatIndex,mainIndex,value}) {
    ...
    const getHeatIndexPercent = () => {
        ...
        if(heatIndex.length)
            setHeatIndexPercent( (((value - HomeConstants.readingRanges.heatIndex.min)) / (HomeConstants.readingRanges.heatIndex.max - HomeConstants.readingRanges.heatIndex.min)).toFixed(1) );
    ...
    return (
        <ReactCardFlip isFlipped={isFlipped} flipDirection="vertical">
                        <motion.div whileHover={{scale: 1.05,}} >
                            ...
                            <h3 className="sbi-label" style={{fontSize:'x-large'}}>Heat Index : {value}</h3>
