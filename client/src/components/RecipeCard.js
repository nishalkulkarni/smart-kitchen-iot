...
function RecipeCard(props) {
  ...
  const gotToOrignal = () => {
    ...
    fetch(
      `https://api.spoonacular.com/recipes/${props.recipe.id}/information?apiKey=${process.env.REACT_APP_SPOON_API_KEY}&includeNutrition=false`
    )
      .then((res) => res.json())
      .then((data) => {
  ...
  return (
    <motion.div
      className={"recipe-card " + (isExpanded ? "expand " : "")}
      initial={{ opacity: 0.9 }}
    ...
    >
      ...
      <motion.div className={"recipe-info-panel "}>
        ...
        {isExpanded && props.title && (
          <h3 className="sbi-label">All Ingredients</h3>
        )}
        {isExpanded &&
          props.title &&
          props.recipe.usedIngredients.map((ing) => {
            return (
              <div style={ingredientInfo}>
                <p className="sbi-label" key={ing.id}>
      ...
      <button
        className={"button-default button-expand "}
