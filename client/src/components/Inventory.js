...
export default function Inventory(props) {
    ...
    function makeIngredQuery(){
        return new Promise( (resolve,reject)=>{
            setIngred(tempList.map(item => {return item.name}));
            if(ingred!=null){
                resolve();
            }
        })
    ...
    function getRecipeByIng(){
        ...
        fetch(`https://api.spoonacular.com/recipes/findByIngredients?apiKey=${process.env.REACT_APP_SPOON_API_KEY}&ingredients=${ingredQuery}&number=5`)
        .then(res => res.json())
        .then(data => {
    ...
    return (
        <Grid item xs={12} >
            ...
            <h2 className="sbi-label" style={{backgroundColor:"white",borderRadius:"15px"}}>Last Synched to <img src="https://firebase.google.com/downloads/brand-guidelines/PNG/logo-standard.png" width="71px" height="20px" ></img> : {lastSynch}</h2>
            <div style={{boxShadow:'5px', border:'2px solid black'}}>
                <GridList cellHeight={160} cols={3} style={inventoryList}>
                    {docs.map(item => (<GridListTile> <h2 className="invlist-label">{item.produce.name}</h2> <b>{item.produce.value} {item.produce.unit}</b> </GridListTile>))}
                </GridList>
            </div>
            <div>
                <button className="button-default" onClick={getAvailableRecipes}>Available Recipes</button>
            </div>
